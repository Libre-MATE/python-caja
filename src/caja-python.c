...
static const guint caja_python_ndebug_keys =
    sizeof(caja_python_debug_keys) / sizeof(GDebugKey);
...
static PyObject *caja_operationhandle_get_handle(PyGBoxed *self,
                                                 void *closure) {
  return PyLong_FromSsize_t((Py_ssize_t)(size_t)self->boxed);
}

static int caja_operationhandle_set_handle(PyGBoxed *self, PyObject *value,
                                           void *closure) {
...
static PyGetSetDef caja_operationhandle_handle = {
    "handle", (getter)caja_operationhandle_get_handle,
    (setter)caja_operationhandle_set_handle, "Operation handle value", NULL};

static inline gboolean np_init_pygobject(void) {
  PyObject *gobject =
      pygobject_init(PYGOBJECT_MAJOR_VERSION, PYGOBJECT_MINOR_VERSION,
...
static void caja_python_load_file(GTypeModule *type_module,
  ...
  while (PyDict_Next(locals, &pos, &key, &value)) {
    ...
    if (PyObject_IsSubclass(value, (PyObject *)&PyCajaColumnProvider_Type) ||
        ...
        PyObject_IsSubclass(value,
                            (PyObject *)&PyCajaPropertyPageProvider_Type)) {
...
static void caja_python_load_dir(GTypeModule *module, const char *dirname) {
  GDir *dir;
  const char *name;
...
static gboolean caja_python_init_python(void) {
  ...
  debug("g_module_open " PY_LIB_LOC "/libpython" PYTHON_VERSION PYTHON_ABIFLAGS
        "." G_MODULE_SUFFIX ".1.0");
  libpython =
      g_module_open(PY_LIB_LOC "/libpython" PYTHON_VERSION PYTHON_ABIFLAGS
                               "." G_MODULE_SUFFIX ".1.0",
                    0);
  if (!libpython)
...
void caja_module_initialize(GTypeModule *module) {
  ...
  if (env_string != NULL) {
    caja_python_debug = g_parse_debug_string(env_string, caja_python_debug_keys,
  ...
  user_extensions_dir = g_build_filename(g_get_user_data_dir(), "caja-python",
                                         "extensions", NULL);
