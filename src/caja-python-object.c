...
#define CHECK_METHOD_NAME(self) \
  if (!PyObject_HasAttrString(self, METHOD_NAME)) goto beach;
...
#define HANDLE_LIST(py_ret, type, type_name)                                   \
  {                                                                            \
    ...
    if (!PySequence_Check(py_ret) || PyUnicode_Check(py_ret)) {                \
      PyErr_SetString(PyExc_TypeError, METHOD_NAME " must return a sequence"); \
      goto beach;                                                              \
    }                                                                          \
    for (i = 0; i < PySequence_Size(py_ret); i++) {                            \
      ...
      if (!pygobject_check(py_item, &Py##type##_Type)) {                       \
        PyErr_SetString(PyExc_TypeError,                                       \
                        METHOD_NAME " must return a sequence of " type_name);  \
        goto beach;                                                            \
      }                                                                        \
...
static void free_pygobject_data(gpointer data, gpointer user_data) {
...
static void free_pygobject_data_list(GList *list) {
  if (list == NULL) return;

  g_list_foreach(list, (GFunc)free_pygobject_data, NULL);
}

static PyObject *caja_python_boxed_new(PyTypeObject *type, gpointer boxed,
...
static GList *caja_python_object_get_property_pages(
...
static void caja_python_object_property_page_provider_iface_init(
    CajaPropertyPageProviderIface *iface) {
  iface->get_pages = caja_python_object_get_property_pages;
}

#define METHOD_NAME "get_widget"
static GtkWidget *caja_python_object_get_widget(
    CajaLocationWidgetProvider *provider, const char *uri, GtkWidget *window) {
  CajaPythonObject *object = (CajaPythonObject *)provider;
  GtkWidget *ret = NULL;
  ...
  py_ret =
      PyObject_CallMethod(object->instance, METHOD_PREFIX METHOD_NAME, "(NN)",
                          py_uri, pygobject_new((GObject *)window));
  HANDLE_RETVAL(py_ret);

  py_ret_gobj = (PyGObject *)py_ret;
  if (!pygobject_check(py_ret_gobj, &PyGtkWidget_Type)) {
    PyErr_SetString(PyExc_TypeError, METHOD_NAME "should return a gtk.Widget");
    goto beach;
  }
...
static void caja_python_object_location_widget_provider_iface_init(
    CajaLocationWidgetProviderIface *iface) {
  iface->get_widget = caja_python_object_get_widget;
}

#define METHOD_NAME "get_file_items"
static GList *caja_python_object_get_file_items(CajaMenuProvider *provider,
                                                GtkWidget *window,
                                                GList *files) {
  CajaPythonObject *object = (CajaPythonObject *)provider;
  GList *ret = NULL;
  ...
  if (PyObject_HasAttrString(object->instance, "get_file_items_full")) {
    ...
    py_ret = PyObject_CallMethod(object->instance,
                                 METHOD_PREFIX "get_file_items_full", "(NNN)",
                                 pygobject_new((GObject *)provider),
                                 pygobject_new((GObject *)window), py_files);
...
static GList *caja_python_object_get_background_items(
    CajaMenuProvider *provider, GtkWidget *window, CajaFileInfo *file) {
  CajaPythonObject *object = (CajaPythonObject *)provider;
  GList *ret = NULL;
  PyObject *py_ret = NULL;
  PyGILState_STATE state = pyg_gil_state_ensure();
  ...
  if (PyObject_HasAttrString(object->instance, "get_background_items_full")) {
    py_ret = PyObject_CallMethod(
        object->instance, METHOD_PREFIX "get_background_items_full", "(NNN)",
        pygobject_new((GObject *)provider), pygobject_new((GObject *)window),
        pygobject_new((GObject *)file));
  } else if (PyObject_HasAttrString(object->instance, "get_background_items")) {
    py_ret = PyObject_CallMethod(object->instance, METHOD_PREFIX METHOD_NAME,
                                 "(NN)", pygobject_new((GObject *)window),
                                 pygobject_new((GObject *)file));
  } else {
    goto beach;
  }
...
static void caja_python_object_menu_provider_iface_init(
...
static GList *caja_python_object_get_columns(CajaColumnProvider *provider) {
  CajaPythonObject *object = (CajaPythonObject *)provider;
  GList *ret = NULL;
  PyObject *py_ret = NULL;
  PyGILState_STATE state = pyg_gil_state_ensure();
  ...
  py_ret =
      PyObject_CallMethod(object->instance, METHOD_PREFIX METHOD_NAME, NULL);
...
beach:
  if (py_ret != NULL) Py_XDECREF(py_ret);
  pyg_gil_state_release(state);
  return ret;
}
#undef METHOD_NAME

static void caja_python_object_column_provider_iface_init(
    CajaColumnProviderIface *iface) {
  iface->get_columns = caja_python_object_get_columns;
}

#define METHOD_NAME "cancel_update"
static void caja_python_object_cancel_update(CajaInfoProvider *provider,
                                             CajaOperationHandle *handle) {
  CajaPythonObject *object = (CajaPythonObject *)provider;
  PyGILState_STATE state = pyg_gil_state_ensure();
  PyObject *py_handle =
      caja_python_boxed_new(_PyCajaOperationHandle_Type, handle, FALSE);
  ...
  PyObject_CallMethod(object->instance, METHOD_PREFIX METHOD_NAME, "(NN)",
                      pygobject_new((GObject *)provider), py_handle);
...
static CajaOperationResult caja_python_object_update_file_info(
    CajaInfoProvider *provider, CajaFile *file, GClosure *update_complete,
    CajaOperationHandle **handle) {
  CajaPythonObject *object = (CajaPythonObject *)provider;
  ...
  *handle = NULL;
  ...
  if (PyObject_HasAttrString(object->instance, "update_file_info_full")) {
    ...
    py_handle = caja_python_boxed_new(_PyCajaOperationHandle_Type, h, FALSE);

    py_ret = PyObject_CallMethod(
        object->instance, METHOD_PREFIX "update_file_info_full", "(NNNN)",
        pygobject_new((GObject *)provider), py_handle,
...
static void caja_python_object_info_provider_iface_init(
...
static void caja_python_object_instance_init(CajaPythonObject *object) {
  ...
  if (object->instance == NULL) PyErr_Print();
}

static void caja_python_object_finalize(GObject *object) {
  debug_enter();
...
static void caja_python_object_class_init(CajaPythonObjectClass *class,
                                          gpointer class_data) {
  debug_enter();
...
GType caja_python_object_get_type(GTypeModule *module, PyObject *type) {
  ...
  static const GInterfaceInfo location_widget_provider_iface_info = {
      (GInterfaceInitFunc)
          caja_python_object_location_widget_provider_iface_init,
      NULL, NULL};

  static const GInterfaceInfo menu_provider_iface_info = {
      (GInterfaceInitFunc)caja_python_object_menu_provider_iface_init, NULL,
      NULL};

  static const GInterfaceInfo column_provider_iface_info = {
      (GInterfaceInitFunc)caja_python_object_column_provider_iface_init, NULL,
      NULL};

  static const GInterfaceInfo info_provider_iface_info = {
      (GInterfaceInitFunc)caja_python_object_info_provider_iface_init, NULL,
      NULL};

  debug_enter_args("type=%s",
  ...
  type_name = g_strdup_printf(
      "%s+CajaPython",
      PyUnicode_AsUTF8(PyObject_GetAttrString(type, "__name__")));

  gtype =
      g_type_module_register_type(module, G_TYPE_OBJECT, type_name, info, 0);

  if (PyObject_IsSubclass(type, (PyObject *)&PyCajaPropertyPageProvider_Type)) {
    g_type_module_add_interface(module, gtype, CAJA_TYPE_PROPERTY_PAGE_PROVIDER,
                                &property_page_provider_iface_info);
  }

  if (PyObject_IsSubclass(type,
                          (PyObject *)&PyCajaLocationWidgetProvider_Type)) {
  ...
  if (PyObject_IsSubclass(type, (PyObject *)&PyCajaMenuProvider_Type)) {
    g_type_module_add_interface(module, gtype, CAJA_TYPE_MENU_PROVIDER,
                                &menu_provider_iface_info);
  }

  if (PyObject_IsSubclass(type, (PyObject *)&PyCajaColumnProvider_Type)) {
    g_type_module_add_interface(module, gtype, CAJA_TYPE_COLUMN_PROVIDER,
                                &column_provider_iface_info);
  }

  if (PyObject_IsSubclass(type, (PyObject *)&PyCajaInfoProvider_Type)) {
    g_type_module_add_interface(module, gtype, CAJA_TYPE_INFO_PROVIDER,
